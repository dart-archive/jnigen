...
public class Main {
    ...
    public static void runDoclet(List<String> qualifiedNames, SummarizerOptions options) {
        ...
        var files =
                javaFilePaths.stream()
                        .flatMap(
                                path -> recursiveListFiles(path, file -> file.getName().endsWith(".java"))
                                        .stream())
                        .map(File::getPath)
                        .toArray(String[]::new);
    ...
    public static void main(String[] args) {
        ...
        if (config.useAsm) {
            ...
            try {
                writeAll(AsmSummarizer.run(config.classPaths.split(File.pathSeparator),
                        cl.getArgs()));
            } catch (IOException e) {
                e.printStackTrace();
            }
    ...
    public static CommandLine parseArgs(String[] args) {
        ...
        Option asm = new Option("A", "use-asm", false, "Use ASM based summarizer and search class path");
        ...
        Option doctoolArgs =
                new Option(
                        "D", "doctool-args", true, "Arguments to pass to the documentation tool");
        Option verbose = new Option("V", "verbose", false, "Enable verbose output");
        Option nonNull =
                new Option("N", "non-null", false, "Print fields only if values are non-null");
        for (Option opt :
                new Option[] {
                    asm,
                    sources,
        ...
        try {
            ...
            help.printHelp(
                    "java -jar <JAR> [-s <SOURCE_DIR=.>] " + "[-c <CLASSES_JAR>] <SOURCE_FILES>",
                    options);
            System.exit(1);
            return null;
        }
